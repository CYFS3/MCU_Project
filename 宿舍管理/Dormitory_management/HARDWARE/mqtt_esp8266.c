...
static char dormitoryId[] = "CYFS";

//电闸Id
const char switch_elecId[10] = "Elec-001";
//水闸Id
const char sluice_waterId[15]= " Sluice-001";

void MQTT_Topic(void)
{
    
    usart_send_string(USART2,(u8*)"AT+MQTTSUB=0,\"control_sluice\",0\r\n");
    delay_Ms(2000);
    usart_send_string(USART2,(u8*)"AT+MQTTSUB=0,\"control_switch\",0\r\n");
...
{
    init_usart2(115200);
    usart_send_string(USART2,(u8*)"AT+CWMODE=1\r\n");
    delay_Ms(2000);
    usart_send_string(USART2,(u8*)"AT+MQTTUSERCFG=0,2,\"0\",\"cyfs\",\"123456\",0,0,\"\"\r\n");
    delay_Ms(2000);
    usart_send_string(USART2,(u8*)"AT+MQTTCONN=0,\"e6ddbc31.ala.cn-hangzhou.emqxsl.cn\",8883,1\r\n");
    delay_Ms(3000);
    MQTT_Topic();
...
void MQTT_Send_LED(char State)
{
    ...
    // 添加字符串字段
    cJSON_AddStringToObject(root, "elecId",switch_elecId);
    cJSON_AddNumberToObject(root, "state",State);    
    // 将 cJSON 对象转换为 JSON 字符串
    char *jsonString = cJSON_Print(root);
    sprintf(send,"AT+MQTTPUBRAW=0,\"access_control\",%d,0,0\r\n",strlen(jsonString));
...
void MQTT_Send_Water(char State)
{
    ...
    // 添加字符串字段
    cJSON_AddStringToObject(root, "waterId",sluice_waterId);
    cJSON_AddNumberToObject(root, "state",State);    
    // 将 cJSON 对象转换为 JSON 字符串
    char *jsonString = cJSON_Print(root);
    sprintf(send,"AT+MQTTPUBRAW=0,\"access_control\",%d,0,0\r\n",strlen(jsonString));
...
void MQTT_Send_Door_Open(void)
{
    ...
    cJSON *root = cJSON_CreateObject();
    // 添加字符串字段
    cJSON_AddStringToObject(root, "dormitoryId",dormitoryId);
...
void MQTT_Send_monitor(int temp,int hum)
{
    ...
    sprintf(send,"AT+MQTTPUBRAW=0,\"room_monitor\",%d,0,0\r\n",strlen(jsonString));
    usart_send_string(USART2,(u8*)send);
    delay_Ms(500);
    usart_send_string(USART2,(u8*)jsonString);
    // 释放 cJSON 对象和 JSON 字符串的内存
