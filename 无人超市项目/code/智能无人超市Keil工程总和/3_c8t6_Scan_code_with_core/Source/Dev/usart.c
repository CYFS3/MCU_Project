...
unsigned char USART1_RecProcess(void)
{
	...
	DisposeUsartTTFT(usart1_rx_buf);//处理串口屏发送过来的数据
...
unsigned char USART2_RecProcess(void)
{
	...
	setScanCode((char*)usart2_rx_buf);
	/*清空接收字符存储区*/
...
unsigned char USART3_RecProcess(void)
{
	...
	/*清空接收字符存储区*/
	DisposeReturnData(usart3_rx_buf);
...
unsigned char UART4_RecProcess(void)
{
	...
	receive_order((char*)uart4_rx_buf);
	/*清空接收字符存储区*/
...
unsigned char USART5_RecProcess(void)
{
