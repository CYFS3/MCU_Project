...
u8 uart4_rx_buf[256];//存放接收文的数据
u16 uart4_pos=0;//最后一个字符位置
u8 uart5_rx_buf[256];//存放接收文的数据
u16 uart5_pos=0;//最后一个字符位置

/*初始化串口1、2、3*/
void usart_init_123(u32 USART1_BaudRate,u32 USART2_BaudRate,u32 USART3_BaudRate,u32 UART4_BaudRate,u32 UART5_BaudRate)
{
	init_usart1(USART1_BaudRate);
	init_usart2(USART2_BaudRate);
	init_usart3(USART3_BaudRate);
	init_uart4(UART4_BaudRate);
	init_uart5(UART5_BaudRate);
}
...
void init_uart4(u32 UART4_BaudRate)
{
	...
	GPIO_InitTypeDef GPIO_struct_TX;//串口发送的GPIO载入的配置
	GPIO_InitTypeDef GPIO_struct_RX;//串口接收的GPIO载入的配置
	USART_InitTypeDef USART4_struct;//串口3的配置
	NVIC_InitTypeDef NVIC_InitStruct;//接收中断配置
	/*串口4-----------------------------------------------------------------*/
	...
	GPIO_struct_TX.GPIO_Speed=GPIO_Speed_50MHz;
	GPIO_struct_TX.GPIO_Mode=GPIO_Mode_AF_PP;
	GPIO_Init(GPIOC,&GPIO_struct_TX);
	/*PC11 UART4_RX 串口4接收*/
	GPIO_struct_RX.GPIO_Pin=GPIO_Pin_11;
	GPIO_struct_RX.GPIO_Speed=GPIO_Speed_50MHz;
	GPIO_struct_RX.GPIO_Mode=GPIO_Mode_IPU;
	GPIO_Init(GPIOC,&GPIO_struct_RX);
	/*初始化并加载串口4配置*/
	USART4_struct.USART_BaudRate=UART4_BaudRate;
	USART4_struct.USART_HardwareFlowControl=USART_HardwareFlowControl_None;
	USART4_struct.USART_Mode=USART_Mode_Rx|USART_Mode_Tx;
	USART4_struct.USART_Parity=USART_Parity_No;
	USART4_struct.USART_StopBits=USART_StopBits_1;
	USART4_struct.USART_WordLength=USART_WordLength_8b;
	USART_Init(UART4,&USART4_struct);
	/*配置串口4中断接收*/
	...
	NVIC_InitStruct.NVIC_IRQChannelSubPriority=2;
	NVIC_Init(&NVIC_InitStruct);
	USART_ITConfig(UART4,USART_IT_RXNE,ENABLE);//开启串口4的接收中断
	USART_Cmd(UART4,ENABLE);//串口4使能
	USART_ClearFlag(UART4,USART_FLAG_TC);//清除接收标志位
}
/*初始化串口5*/
void init_uart5(u32 UART5_BaudRate)
{
	...
	GPIO_InitTypeDef GPIO_struct_TX;//串口发送的GPIO载入的配置
	GPIO_InitTypeDef GPIO_struct_RX;//串口接收的GPIO载入的配置
	USART_InitTypeDef USART5_struct;//串口5的配置
	NVIC_InitTypeDef NVIC_InitStruct;//接收中断配置
	/*串口5-----------------------------------------------------------------*/
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC|RCC_APB2Periph_GPIOD,ENABLE);
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_UART5,ENABLE);
	/*PC12 UART5_TX 串口5发送*/
	GPIO_struct_TX.GPIO_Pin=GPIO_Pin_12;
	GPIO_struct_TX.GPIO_Speed=GPIO_Speed_50MHz;
	GPIO_struct_TX.GPIO_Mode=GPIO_Mode_AF_PP;
	GPIO_Init(GPIOC,&GPIO_struct_TX);
	/*PD2 UART5_RX 串口5接收*/
	GPIO_struct_RX.GPIO_Pin=GPIO_Pin_2;
	GPIO_struct_RX.GPIO_Speed=GPIO_Speed_50MHz;
	GPIO_struct_RX.GPIO_Mode=GPIO_Mode_IPU;
	GPIO_Init(GPIOD,&GPIO_struct_RX);
	/*初始化并加载串口5配置*/
	USART5_struct.USART_BaudRate=UART5_BaudRate;
	USART5_struct.USART_HardwareFlowControl=USART_HardwareFlowControl_None;
	USART5_struct.USART_Mode=USART_Mode_Rx|USART_Mode_Tx;
	USART5_struct.USART_Parity=USART_Parity_No;
	USART5_struct.USART_StopBits=USART_StopBits_1;
	USART5_struct.USART_WordLength=USART_WordLength_8b;
	USART_Init(UART5,&USART5_struct);
	/*配置串口5中断接收*/
	...
	NVIC_InitStruct.NVIC_IRQChannelSubPriority=2;
	NVIC_Init(&NVIC_InitStruct);
	USART_ITConfig(UART5,USART_IT_RXNE,ENABLE);//开启串口5的接收中断
	USART_Cmd(UART5,ENABLE);//串口5使能
	USART_ClearFlag(UART5,USART_FLAG_TC);//清除接收标志位
}
...
void UART4_IRQHandler(void)
{
	...
	{
		UART4RecTimer=10;
		RecCh=(u8)(USART_ReceiveData(UART4));
		uart4_rx_buf[uart4_pos++]=RecCh;
	}
...
unsigned char UART4_RecProcess(void)
{
	...
	if(!uart4_pos)return false;
	uart4_rx_buf[uart4_pos]='\0';
	UART4_Receive(uart4_rx_buf);
	/*清空接收字符存储区*/
	memset(uart4_rx_buf,0,256);
	uart4_pos=0;
	return true;
}
...
unsigned char UART5_RecProcess(void)
{
	...
	if(!uart5_pos)return false;
	uart5_rx_buf[uart5_pos]='\0';
	UART5_Receive(uart5_rx_buf);
	/*清空接收字符存储区*/
	memset(uart5_rx_buf,0,256);
	uart5_pos=0;
	return true;
}
